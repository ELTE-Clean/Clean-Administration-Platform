...
router.delete(
  ...
  async (req, res, next) => {
    if (!req.body.taskID)
      return res
        .status(400)
        .send(JSON.stringify({ message: "taskID not found" }));
    ...
    return res
      .status(200)
      .send(JSON.stringify({ message: "Task successfully deleted" }));
  }
);
  ...
  async (req, res, next) => {
    ...
    const params = {
      solution:
        ...
        oldTask.solution,
      description:
        req.files?.description.data.toString("utf8") ||
...
router.post(
  ...
  async (req, res, next) => {
    ...
    if (taskRecord.error) return next(taskRecord.error);
    if (taskRecord.result.rows.length < 1)
      return res
        .status(404)
        .send(JSON.stringify({ message: "Task does not exist" }));
    if (!taskRecord.result.rows[0].solution)
      return res
        .status(404)
        .send(JSON.stringify({ message: "No teacher solution is given" }));

    const studentSolutionRecord = await selectFromTable("grades", {
      taskID: tid,
    });
    if (studentSolutionRecord.error) return next(studentSolutionRecord.error);
    ...
    for (let dir of directories) {
      accum += dir + "/";
      if (fs.existsSync(accum)) continue;
    ...
    fs.writeFileSync(
      taskDir + "teacher.icl",
      taskRecord.result.rows[0].solution,
      (err) => {
    ...
    const scriptCode =
      "python ./scripts/scripts/CorrectTest.py -f " + configPath + " -tn 1";
